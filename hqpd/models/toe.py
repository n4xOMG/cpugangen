...
class TagOptimizedEncoder(nn.Module):
    ...
    def __init__(
        ...
        enable_pooling=False  # NEW: Enable pooling head for SDXL compatibility
    ):
        super().__init__()
        ...
        # NEW: Pooling head for SDXL pooled embeddings (1280-dim)
        if enable_pooling:
            self.pooling_head = nn.Sequential(
                nn.Linear(embed_dim, 1280),
                nn.LayerNorm(1280)
            )
        else:
    ...
    def forward(self, tag_ids, tag_weights=None, return_pooled=False):
        """
        ...
        Args:
            ...
            return_pooled: If True, return (context, pooled) tuple
        
        Returns:
            context: (batch_size, max_length, embed_dim) - Context embeddings
            pooled: (batch_size, 1280) - Pooled embeddings (if return_pooled=True and pooling_head exists)
        """
        ...
        # Generate pooled embeddings if requested
        if return_pooled and self.pooling_head is not None:
            # Mean pooling over sequence dimension
            pooled = context.mean(dim=1)  # (batch, embed_dim)
            pooled = self.pooling_head(pooled)  # (batch, 1280)
